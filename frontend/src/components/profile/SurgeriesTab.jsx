...
import { surgeriesSchema } from "../../schemas/profileSchema";
...
export default function SurgeriesTab({ user }) {
  ...
  const {
    register,
    control, // 👉 REQUIRED for arrays of objects
    handleSubmit,
    reset,
    getValues,
    formState: { errors },
  ...
  const onSubmit = (data) => {
    ...
    alert("Surgeries saved!");
    setIsEditing(false);
  };

  const handleCancel = () => {
    reset(); // Reverts to baseline, throwing away unsaved new rows
    setIsEditing(false);
  };

  const handleAddSurgery = () => {
    append({ surgeryName: "", year: "", hospital: "" });
  };
  ...
  return (
    <div>
      ...
      {/* Dynamic List of Surgeries */}
      <div className="space-y-4">
        {fields.length === 0 && !isEditing && (
          <p className="text-sm opacity-60">No past surgeries listed.</p>
        )}
        ...
        {fields.map((item, index) => (
          ...
          >
            <Input
              ...
              {...register(`surgeries.${index}.surgeryName`)}
              error={errors?.surgeries?.[index]?.surgeryName}
              disabled={!isEditing}
            />
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input
                ...
                // 👉 valueAsNumber ensures it saves to the DB as an integer, not a string
                {...register(`surgeries.${index}.year`, {
                  valueAsNumber: true,
                })}
                error={errors?.surgeries?.[index]?.year}
                disabled={!isEditing}
              />
              <Input
                label="Hospital/Clinic"
                placeholder="e.g. City General"
                {...register(`surgeries.${index}.hospital`)}
                error={errors?.surgeries?.[index]?.hospital}
                disabled={!isEditing}
              />
            </div>
